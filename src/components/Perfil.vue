<template>
  <div class="perfil p-6 bg-white rounded-lg shadow-lg">
    <h2 class="text-2xl sm:text-xl font-bold mb-6 text-cyan-500">Perfil de usuario</h2>
    ...
    <div v-else>
      ...
      <div class="flex flex-col md:flex-row items-center justify-center mb-6">
        <div class="perfil-imagen mr-4 mb-4 md:mb-0">
          <img v-if="userPhotoURL" :src="userPhotoURL" alt="Foto de perfil"
            class="rounded-full w-28 h-28 object-cover border-4 border-cyan-400 shadow-md">
      ...
      <div v-if="mostrarFormulario" class="mt-6 bg-gray-50 p-6 rounded-lg shadow-md">
        <h3 class="text-x2 sm:text-lg font-semibold mb-4 text-center text-cyan-500">Editar Perfil</h3>
        <form @submit.prevent="actualizarDatos">
          ...
          <div class="mb-4">
            <label for="password" class="block text-sm sm:text-base font-medium text-gray-700">Nueva Contraseña:</label>
      ...
      <div class="mt-6">
        <h3 class="text-xl sm:text-lg font-semibold mb-4 text-cyan-500">Mis Publicaciones</h3>
        <div v-for="publicacion in publicaciones" :key="publicacion.id"
          ...
          <h3 class="text-lg sm:text-base font-semibold text-blue-500">{{ publicacion.titulo }}</h3>
