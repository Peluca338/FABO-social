<template>
  <div class="modal-overlay">
    <div class="modal-content bg-white rounded shadow p-4">
      <h2 class="text-2xl font-bold mb-4">Perfil de {{ publicacion.username || 'Cargando...' }}</h2>

      <!-- Imagen de perfil y datos del usuario -->
      <div class="flex items-center mb-4">
        <div class="perfil-imagen mr-4">
          <img v-if="publicacion.fotoPerfil" :src="publicacion.fotoPerfil" alt="Foto de perfil" class="rounded-full w-24 h-24 object-cover">
      ...
      <button @click="$emit('cerrar')" class="mt-4 text-red-500">Cerrar</button>
    </div>
  </div>
</template>
