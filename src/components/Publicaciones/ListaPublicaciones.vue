  ...
  <div class="container mx-auto p-4 sm:p-6 md:max-w-3xl lg:max-w-5xl">
    <div class="flex flex-col lg:flex-row gap-6">
      ...
      <div class="w-full lg:w-2/3 flex flex-col">
        <div class="bg-gray-50 p-4 sm:p-6 rounded-lg shadow-lg flex-grow">
          ...
          <div v-else>
            <div v-for="publicacion in publicaciones" :key="publicacion.id"
              class="mb-4 sm:mb-6 p-4 sm:p-6 border border-gray-200 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-200 bg-white">
              <!-- Publicación Content -->
              <div class="flex items-center justify-between mb-4 space-x-4">
                <!-- Información del Usuario -->
                <div class="flex items-center space-x-4 cursor-pointer" @click="abrirModalPerfil(publicacion)">
                  <img :src="publicacion.fotoPerfil || '/public/img/user.png'"
                    class="w-12 h-12 sm:w-14 sm:h-14 rounded-full" alt="perfil">
                  <div>
              ...
              <div class="flex justify-between items-center mt-4" v-if="esPropietario(publicacion.userId)">
                <!-- Botón de Editar -->
                <button @click="editarPublicacion(publicacion.id)"
                  class="flex items-center space-x-2 px-3 py-1 bg-amber-500 text-white rounded-md hover:bg-amber-400 transition text-xs sm:text-sm">
      ...
      <!-- Chat Section -->
      <div class="w-full lg:w-1/3 bg-gray-100 p-4 sm:p-6 rounded-lg shadow-lg flex-shrink-0">
        <Chat :userId="userId" />
      </div>
    </div>
...
export default {
  components: { Comentarios, Chat, ModalPerfil, Loader },
  data() {
    return {
      publicaciones: [],
      userId: null,
  ...
  methods: {
    async obtenerPublicaciones() {
      try {
        ...
        this.publicaciones = await Promise.all(publicacionesSnapshot.docs.map(async (doc) => {
          const data = doc.data();
          // Obtener la URL de descarga de la foto de perfil si está disponible
          let fotoPerfil = data.fotoPerfil || '/public/img/user.png'; // Fallback si no existe fotoPerfil
          if (fotoPerfil !== '/public/img/user.png') {
          ...
          return { id: doc.id, ...data, fotoPerfil };
        }));
        this.cargando = false; 
      } catch (error) {
